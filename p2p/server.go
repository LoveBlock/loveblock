...
// Package p2p implements the LoveBlock p2p network protocols.
package p2p
...
import (
	...
	"github.com/LoveBlock/loveblock/p2p/netutil"
)
...
func (srv *Server) Peers() []*Peer {
	var ps []*Peer
	select {
	// Note: We'd network to put this function into a variable but
	...
	if !srv.NoDiscovery || srv.DiscoveryV5 {
		...
		if srv.NAT != nil {
			if !realaddr.IP.IsLoopback() {
				go nat.Map(srv.NAT, srv.quit, "udp", realaddr.Port, realaddr.Port, "loveblock discovery")
			}
...
func (srv *Server) startListening() error {
	...
	if !laddr.IP.IsLoopback() && srv.NAT != nil {
		srv.loopWG.Add(1)
		go func() {
			nat.Map(srv.NAT, srv.quit, "tcp", laddr.Port, laddr.Port, "loveblock p2p")
			srv.loopWG.Done()
		}()
	}
	return nil
