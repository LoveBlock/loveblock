...
const (
	...
	URN_WANLemoernetLinkConfig_1   = "urn:schemas-upnp-org:service:WANLemoernetLinkConfig:1"
...
//
// * NewLinkType: allowed values: Lemoernet
...
// Return values:
//
// * NewWANAccessType: allowed values: DSL, POTS, Cable, Lemoernet
//
...
// WANLemoernetLinkConfig1 is a client for UPnP SOAP service with URN "urn:schemas-upnp-org:service:WANLemoernetLinkConfig:1". See
// goupnp.ServiceClient, which contains RootDevice and Service attributes which
// are provided for informational value.
type WANLemoernetLinkConfig1 struct {
	goupnp.ServiceClient
}

// NewWANLemoernetLinkConfig1Clients discovers instances of the service on the network,
// and returns clients to any that are found. errors will contain an error for
// any devices that replied but which could not be queried, and err will be set
// if the discovery process failed outright.
//
// This is a typical entry calling point into this package.
func NewWANLemoernetLinkConfig1Clients() (clients []*WANLemoernetLinkConfig1, errors []error, err error) {
	var genericClients []goupnp.ServiceClient
	if genericClients, errors, err = goupnp.NewServiceClients(URN_WANLemoernetLinkConfig_1); err != nil {
		return
	}
	clients = newWANLemoernetLinkConfig1ClientsFromGenericClients(genericClients)
	return
}

// NewWANLemoernetLinkConfig1ClientsByURL discovers instances of the service at the given
// URL, and returns clients to any that are found. An error is returned if
// there was an error probing the service.
//
// This is a typical entry calling point into this package when reusing an
// previously discovered service URL.
func NewWANLemoernetLinkConfig1ClientsByURL(loc *url.URL) ([]*WANLemoernetLinkConfig1, error) {
	genericClients, err := goupnp.NewServiceClientsByURL(loc, URN_WANLemoernetLinkConfig_1)
	if err != nil {
		return nil, err
	}
	return newWANLemoernetLinkConfig1ClientsFromGenericClients(genericClients), nil
}

// NewWANLemoernetLinkConfig1ClientsFromRootDevice discovers instances of the service in
// a given root device, and returns clients to any that are found. An error is
// returned if there was not at least one instance of the service within the
// device. The location parameter is simply assigned to the Location attribute
// of the wrapped ServiceClient(s).
...
// This is a typical entry calling point into this package when reusing an
// previously discovered root device.
func NewWANLemoernetLinkConfig1ClientsFromRootDevice(rootDevice *goupnp.RootDevice, loc *url.URL) ([]*WANLemoernetLinkConfig1, error) {
	genericClients, err := goupnp.NewServiceClientsFromRootDevice(rootDevice, loc, URN_WANLemoernetLinkConfig_1)
	if err != nil {
		return nil, err
	}
	return newWANLemoernetLinkConfig1ClientsFromGenericClients(genericClients), nil
...
func newWANLemoernetLinkConfig1ClientsFromGenericClients(genericClients []goupnp.ServiceClient) []*WANLemoernetLinkConfig1 {
	clients := make([]*WANLemoernetLinkConfig1, len(genericClients))
	for i := range genericClients {
		clients[i] = &WANLemoernetLinkConfig1{genericClients[i]}
	}
	return clients
}
...
// Return values:
//
// * NewLemoernetLinkStatus: allowed values: Up, Down
func (client *WANLemoernetLinkConfig1) GetLemoernetLinkStatus() (NewLemoernetLinkStatus string, err error) {
	// Request structure.
	request := interface{}(nil)
	// BEGIN Marshal arguments into request.
	...
	// Response structure.
	response := &struct {
		NewLemoernetLinkStatus string
	}{}

	// Perform the SOAP call.
	if err = client.SOAPClient.PerformAction(URN_WANLemoernetLinkConfig_1, "GetLemoernetLinkStatus", request, response); err != nil {
		return
	}

	// BEGIN Unmarshal arguments from response.

	if NewLemoernetLinkStatus, err = soap.UnmarshalString(response.NewLemoernetLinkStatus); err != nil {
		return
	}
	// END Unmarshal arguments from response.
	return
