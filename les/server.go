...
func NewLesServer(network *network.Loveblock, config *network.Config) (*LesServer, error) {
	quitSync := make(chan struct{})
	pm, err := NewProtocolManager(network.BlockChain().Config(), false, ServerProtocolVersions, config.NetworkId, network.EventMux(), network.Engine(), newPeerSet(), network.BlockChain(), network.TxPool(), network.ChainDb(), nil, nil, quitSync, new(sync.WaitGroup))
	if err != nil {
		return nil, err
	}
	...
	for i, pv := range AdvertiseProtocolVersions {
		lesTopics[i] = lesTopic(network.BlockChain().Genesis().Hash(), pv)
	}
	...
	srv := &LesServer{
		...
		quitSync:         quitSync,
		lesTopics:        lesTopics,
		chtIndexer:       light.NewChtIndexer(network.ChainDb(), false),
		bloomTrieIndexer: light.NewBloomTrieIndexer(network.ChainDb(), false),
	}
	logger := log.New()
	...
	srv.chtIndexer.Start(network.BlockChain())
	pm.server = srv
	...
	srv.fcCostStats = newCostStats(network.ChainDb())
	return srv, nil
}
...
func newCostStats(db lovedb.Database) *requestCostStats {
