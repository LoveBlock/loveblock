...
func newTester(t *testing.T, confOverride func(*network.Config)) *tester {
	...
	// Create a networkless protocol stack and start an LoveBlock service within
	stack, err := node.New(&node.Config{DataDir: workspace, UseLightweightKDF: true, Name: testInstance})
	if err != nil {
		t.Fatalf("failed to create node: %v", err)
	}
	networkConf := &network.Config{
		Genesis:  core.DeveloperGenesisBlock(15, common.Address{}),
		Lovebase: common.HexToAddress(testAddress),
	}
	if confOverride != nil {
		confOverride(networkConf)
	}
	if err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) { return network.New(ctx, networkConf) }); err != nil {
		t.Fatalf("failed to register Loveblock protocol: %v", err)
	}
	...
	var loveblock *network.Loveblock
	stack.Service(&loveblock)
