...
import (
	...
	"github.com/LoveBlock/loveblock/common/bitutil"
...
var (
	...
	testnetCheckpoint = trustedCheckpoint{
		name:          "testnet",
		sectionIdx:    83,
...
var trustedCheckpoints = map[common.Hash]trustedCheckpoint{
	...
	params.TestnetGenesisHash: testnetCheckpoint,
}
...
func GetChtRoot(db lovedb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
...
func GetChtV2Root(db lovedb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
	return GetChtRoot(db, (sectionIdx+1)*(CHTFrequencyClient/CHTFrequencyServer)-1, sectionHead)
}

// StoreChtRoot writes the CHT root assoctiated to the given section into the database
// Note that sectionIdx is specified according to LES/1 CHT section size
func StoreChtRoot(db lovedb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func NewChtIndexer(db lovedb.Database, clientMode bool) *core.ChainIndexer {
	...
	idb := lovedb.NewTable(db, "chtIndex-")
	backend := &ChtIndexerBackend{
		diskdb:      db,
		triedb:      trie.NewDatabase(lovedb.NewTable(db, ChtTablePrefix)),
...
const (
	...
	networkBloomBitsSection       = 4096
	networkBloomBitsConfirmations = 256
)
...
func GetBloomTrieRoot(db lovedb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func StoreBloomTrieRoot(db lovedb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
// NewBloomTrieIndexer creates a BloomTrie chain indexer
func NewBloomTrieIndexer(db lovedb.Database, clientMode bool) *core.ChainIndexer {
	backend := &BloomTrieIndexerBackend{
		diskdb: db,
		triedb: trie.NewDatabase(lovedb.NewTable(db, BloomTrieTablePrefix)),
	}
	idb := lovedb.NewTable(db, "bltIndex-")
	...
	if clientMode {
		...
		backend.parentSectionSize = networkBloomBitsSection
		confirmReq = HelperTrieProcessConfirmations
	}
	backend.bloomTrieRatio = BloomTrieFrequency / backend.parentSectionSize
	backend.sectionHeads = make([]common.Hash, backend.bloomTrieRatio)
	return core.NewChainIndexer(db, idb, backend, BloomTrieFrequency, confirmReq-networkBloomBitsConfirmations, time.Millisecond*100, "bloomtrie")
}
