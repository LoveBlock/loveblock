...
func TestTxPool(t *testing.T) {
	for i := range testTx {
		testTx[i], _ = types.SignTx(types.NewTransaction(uint64(i), acc1Addr, big.NewInt(10000), params.TxGas, nil, nil), types.NewDefaultSigner(params.AllLovehashProtocolChanges.ChainId), testBankKey)
	}
	...
	gchain, _ := core.GenerateChain(params.TestChainConfig, genesis, dpovp.NewFaker(), sdb, poolTestBlocks, txPoolTestChainGen)
	...
	lightchain, _ := NewLightChain(odr, params.TestChainConfig, dpovp.NewFullFaker())
	txPermanent = 50
