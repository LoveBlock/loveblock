...
func TestNodeIterator(t *testing.T) {
	var (
		fulldb, _  = lovedb.NewMemDatabase()
		lightdb, _ = lovedb.NewMemDatabase()
		gspec      = core.Genesis{Alloc: core.GenesisAlloc{testBankAddress: {Balance: testBankFunds}}}
		genesis    = gspec.MustCommit(fulldb)
	)
	gspec.MustCommit(lightdb)
	blockchain, _ := core.NewBlockChain(fulldb, nil, params.TestChainConfig, dpovp.NewFullFaker(), vm.Config{})
	gchain, _ := core.GenerateChain(params.TestChainConfig, genesis, dpovp.NewFaker(), fulldb, 4, testChainGen)
