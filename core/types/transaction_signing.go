...
func MakeSigner(config *params.ChainConfig) Signer {
	return NewDefaultSigner(config.ChainId)
}
...
func NewDefaultSigner(chainId *big.Int) DefaultSigner {
	if chainId == nil {
		chainId = new(big.Int)
	}
...
func (s DefaultSigner) Sender(tx *Transaction) (common.Address, error) {
...
func (s DefaultSigner) SignatureValues(tx *Transaction, sig []byte) (R, S, V *big.Int, err error) {
	if len(sig) != 65 {
...
func deriveChainId(v *big.Int) *big.Int {
	// if v.BitLen() <= 64 {
	// 	return new(big.Int).SetUint64((v.Uint64() - 35) / 2)
	// }
