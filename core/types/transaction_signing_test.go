...
func TestDefaultSigning(t *testing.T) {
	...
	signer := NewDefaultSigner(big.NewInt(18))
...
func TestDefaultSigningVitalik(t *testing.T) {
	...
	for i, test := range []struct {
		...
		signer := NewDefaultSigner(big.NewInt(1))
...
func TestChainId(t *testing.T) {
	...
	var err error
	tx, err = SignTx(tx, NewDefaultSigner(big.NewInt(1)), key)
	if err != nil {
		t.Fatal(err)
	}

	_, err = Sender(NewDefaultSigner(big.NewInt(2)), tx)
	if err != ErrInvalidChainId {
		t.Error("expected error:", ErrInvalidChainId)
	}

	_, err = Sender(NewDefaultSigner(big.NewInt(1)), tx)
	if err != nil {
