...
func SetupGenesisBlock(db lovedb.Database, genesis *Genesis) (*params.ChainConfig, common.Hash, error) {
	if genesis != nil && genesis.Config == nil {
		return params.AllLovehashProtocolChanges, common.Hash{}, errGenesisNoConfig
	}
	...
	_, err := GetChainConfig(db, stored)
	if err != nil {
...
func (g *Genesis) configOrDefault(ghash common.Hash) *params.ChainConfig {
	...
	default:
		return params.AllLovehashProtocolChanges
	}
}

// ToBlock creates the genesis block and writes state of a genesis specification
// to the given database (or discards it if nil).
func (g *Genesis) ToBlock(db lovedb.Database) *types.Block {
	if db == nil {
...
func (g *Genesis) Commit(db lovedb.Database) (*types.Block, error) {
	block := g.ToBlock(db)
	...
	config := g.Config
	if config == nil {
		config = params.AllLovehashProtocolChanges
	}
...
// The block is committed as the canonical head block.
func (g *Genesis) MustCommit(db lovedb.Database) *types.Block {
...
func GenesisBlockForTesting(db lovedb.Database, addr common.Address, balance *big.Int) *types.Block {
	g := Genesis{Alloc: GenesisAlloc{addr: {Balance: balance}}}
	return g.MustCommit(db)
}

// DefaultGenesisBlock returns the LoveBlock main net genesis block.
...
// DeveloperGenesisBlock returns the 'loveblock --dev' genesis block. Note, this must
// be seeded with the
