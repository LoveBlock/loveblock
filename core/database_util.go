...
var (
	...
	configPrefix   = []byte("loveblock-config-") // config prefix for the db
...
func WriteCanonicalHash(db lovedb.Putter, hash common.Hash, number uint64) error {
...
func WriteHeadHeaderHash(db lovedb.Putter, hash common.Hash) error {
...
func WriteHeadBlockHash(db lovedb.Putter, hash common.Hash) error {
...
func WriteHeadFastBlockHash(db lovedb.Putter, hash common.Hash) error {
...
func WriteTrieSyncProgress(db lovedb.Putter, count uint64) error {
...
func WriteHeader(db lovedb.Putter, header *types.Header) error {
...
func WriteBody(db lovedb.Putter, hash common.Hash, number uint64, body *types.Body) error {
...
func WriteBodyRLP(db lovedb.Putter, hash common.Hash, number uint64, rlp rlp.RawValue) error {
...
func WriteTd(db lovedb.Putter, hash common.Hash, number uint64, td *big.Int) error {
...
func WriteBlock(db lovedb.Putter, block *types.Block) error {
...
func WriteBlockReceipts(db lovedb.Putter, hash common.Hash, number uint64, receipts types.Receipts) error {
...
func WriteTxLookupEntries(db lovedb.Putter, block *types.Block) error {
...
func WriteBloomBits(db lovedb.Putter, bit uint, section uint64, head common.Hash, bits []byte) {
	key := append(append(bloomBitsPrefix, make([]byte, 10)...), head.Bytes()...)
...
func PreimageTable(db lovedb.Database) lovedb.Database {
	return lovedb.NewTable(db, preimagePrefix)
}

// WritePreimages writes the provided set of preimages to the database. `number` is the
// current block number, and is used for debug messages only.
func WritePreimages(db lovedb.Database, number uint64, preimages map[common.Hash][]byte) error {
...
func WriteBlockChainVersion(db lovedb.Putter, vsn int) {
	enc, _ := rlp.EncodeToBytes(uint(vsn))
	db.Put([]byte("BlockchainVersion"), enc)
}

// WriteChainConfig writes the chain config settings to the database.
func WriteChainConfig(db lovedb.Putter, hash common.Hash, cfg *params.ChainConfig) error {
