...
type TxPoolConfig struct {
	NoLocals  bool          // Whnetwork local transaction handling should be disabled
...
func NewTxPool(config TxPoolConfig, chainconfig *params.ChainConfig, chain blockChain) *TxPool {
	...
	pool := &TxPool{
		...
		chain:       chain,
		signer:      types.NewDefaultSigner(chainconfig.ChainId),
...
func (pool *TxPool) addTxsLocked(txs []*types.Transaction, local bool) []error {
	...
	// Only reprocess the internal state if somnetworking was actually added
	if len(dirty) > 0 {
