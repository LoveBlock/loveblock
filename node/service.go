...
func (ctx *ServiceContext) OpenDatabase(name string, cache int, handles int) (lovedb.Database, error) {
	if ctx.config.DataDir == "" {
		return lovedb.NewMemDatabase()
	}
	db, err := lovedb.NewLDBDatabase(ctx.config.resolvePath(name), cache, handles)
	if err != nil {
		return nil, err
	}
	return db, nil
