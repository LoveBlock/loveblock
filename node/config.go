...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of loveblock is "loveblock". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Gnetwork", keep that.
	if name == "loveblock" || name == "loveblock-testnet" {
		name = "Gnetwork"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "loveblock" instances.
var isOldGnetworkResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) resolvePath(path string) string {
	...
	// by loveblock 1.4 are used if they exist.
	if c.name() == "loveblock" && isOldGnetworkResource[path] {
		oldpath := ""
		if c.Name == "loveblock" {
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "loveblock-keystore")
		ephemeral = keydir
	}
