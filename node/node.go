...
func New(conf *Config) (*Node, error) {
	...
	// We rely on this in cmd/loveblock.
...
func (n *Node) OpenDatabase(name string, cache, handles int) (lovedb.Database, error) {
	if n.config.DataDir == "" {
		return lovedb.NewMemDatabase()
	}
	return lovedb.NewLDBDatabase(n.config.resolvePath(name), cache, handles)
}
	...
	return []rpc.API{
		{
			...
			Version:   "1.0",
			Service:   NewPublicLoveClientAPI(n),
			Public:    true,
		},
	}
}
