...
type Fetcher struct {
	...
	fetchingHook       func([]common.Hash)     // Method to call upon starting a block (network/61) or header (network/62) fetch
	completingHook     func([]common.Hash)     // Method to call upon starting a block body fetch (network/62)
	importedHook       func(*types.Block)      // Method to call upon successful block import (both network/61 and network/62)
}
...
func (f *Fetcher) FilterHeaders(peer string, headers []*types.Header, time time.Time) []*types.Header {
	...
	// filter的读取端必须处理完成，再往filter里传输新的才能继续 否则阻塞在这
...
func (f *Fetcher) insert(peer string, block *types.Block) {
	...
	go func() {
		...
		default:
			// Somnetworking went very wrong, drop the peer
