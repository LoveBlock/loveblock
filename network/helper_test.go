...
func newTestProtocolManager(mode downloader.SyncMode, blocks int, generator func(int, *core.BlockGen), newtx chan<- []*types.Transaction) (*ProtocolManager, *lovedb.MemDatabase, error) {
	var (
	...
	chain, _ := core.GenerateChain(gspec.Config, genesis, dpovp.NewFaker(), db, blocks, generator)
...
// fails the test.
func newTestProtocolManagerMust(t *testing.T, mode downloader.SyncMode, blocks int, generator func(int, *core.BlockGen), newtx chan<- []*types.Transaction) (*ProtocolManager, *lovedb.MemDatabase) {
...
func (p *testTxPool) Pending() (map[common.Address]types.Transactions, error) {
	...
	for _, tx := range p.pool {
		from, _ := types.Sender(types.NewDefaultSigner(params.AllLovehashProtocolChanges.ChainId), tx)
...
func newTestTransaction(from *ecdsa.PrivateKey, nonce uint64, datasize int) *types.Transaction {
	...
	tx, _ = types.SignTx(tx, types.NewDefaultSigner(params.AllLovehashProtocolChanges.ChainId), from)
	return tx
}
