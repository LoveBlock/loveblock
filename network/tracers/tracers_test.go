...
import (
	...
	"github.com/LoveBlock/loveblock/tests"
)

// To generate a new callTracer test, copy paste the makeTest method below into
// a Gnetwork console and call it with a transaction hash you which to export.
...
var makeTest = function(tx, rewind) {
  ...
  var block   = network.getBlock(network.getTransaction(tx).blockHash);
  var genesis = network.getBlock(block.parentHash);
  ...
  genesis.config = admin.nodeInfo.protocols.network.config;
  ...
  console.log(JSON.stringify({
    ...
    input:  network.getRawTransaction(tx),
    result: result,
  }, null, 2));
}
*/
