...
const (
	// bloomServiceThreads is the number of goroutines used globally by an LoveBlock
...
func (network *Loveblock) startBloomHandlers() {
	for i := 0; i < bloomServiceThreads; i++ {
		go func() {
			for {
				select {
				case <-network.shutdownChan:
					return

				case request := <-network.bloomRequests:
					task := <-request
					task.Bitsets = make([][]byte, len(task.Sections))
					for i, section := range task.Sections {
						head := core.GetCanonicalHash(network.chainDb, (section+1)*params.BloomBitsBlocks-1)
						if compVector, err := core.GetBloomBits(network.chainDb, task.Bit, section, head); err == nil {
...
// for the LoveBlock header bloom filters, permitting blazing fast filtering.
type BloomIndexer struct {
	size uint64 // section size to generate bloombits for

	db  lovedb.Database      // database instance to write index data and metadata into
...
func NewBloomIndexer(db lovedb.Database, size uint64) *core.ChainIndexer {
	...
	table := lovedb.NewTable(db, string(core.BloomBitsIndexPrefix))
