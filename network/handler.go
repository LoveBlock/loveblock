...
import (
	...
	"github.com/LoveBlock/loveblock/network/fetcher"
...
func NewProtocolManager(config *params.ChainConfig, mode downloader.SyncMode, networkId uint64, mux *event.TypeMux, txpool txPool, engine consensus.Engine, blockchain *core.BlockChain, chaindb lovedb.Database) (*ProtocolManager, error) {
	...
	for i, version := range ProtocolVersions {
		...
		if mode == downloader.FastSync && version < network63 {
			continue
		}
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing Loveblock peer", "peer", id)

	// Unregister the peer from the downloader and LoveBlock peer set
...
func (pm *ProtocolManager) Stop() {
	log.Info("Stopping Loveblock protocol")
	...
	log.Info("Loveblock protocol stopped")
}
...
// handle is the callback invoked to manage the life cycle of an network peer. When
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Loveblock peer connected", "name", p.Name())

	// Execute the LoveBlock handshake
	var (
	...
	if err := p.Handshake(pm.networkId, td, hash, genesis.Hash()); err != nil {
		p.Log().Debug("Loveblock handshake failed", "err", err)
		return err
	}
	...
	if dpovp.GetCoreNodeIndexByPubkey(pubKey) == -1 { // 不在主节点中
		...
		if err := pm.peersDelay.Register(p); err != nil {
			p.Log().Error("Loveblock peer registration failed", "err", err)
			return err
		}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Loveblock message handling failed", "err", err)
			return err
		}
	}
}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	case p.version >= network63 && msg.Code == GetNodeDataMsg:
		// Decode the retrieval message
		msgStream := rlp.NewStream(msg.Payload, uint64(msg.Size))
		if _, err := msgStream.List(); err != nil {
			return err
	...
	case p.version >= network63 && msg.Code == NodeDataMsg:
	...
	case p.version >= network63 && msg.Code == GetReceiptsMsg:
		// Decode the retrieval message
		msgStream := rlp.NewStream(msg.Payload, uint64(msg.Size))
		if _, err := msgStream.List(); err != nil {
			return err
	...
	case p.version >= network63 && msg.Code == ReceiptsMsg:
	...
	case msg.Code == NewConsensusMsg: // sman for consensus message
		...
		for _, msg := range announces {
			pm.blockchain.ProcConsensusMsg(msg, crypto.FromECDSAPub(p.Pubkey))
		}
