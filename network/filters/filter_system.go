...
// Package filters implements an LoveBlock filtering system for block,
...
type subscription struct {
	...
	typ       Type
	created   time.Time
	logsCrit  loveblock.FilterQuery
	logs      chan []*types.Log
...
func (es *EventSystem) SubscribeLogs(crit loveblock.FilterQuery, logs chan []*types.Log) (*Subscription, error) {
...
func (es *EventSystem) subscribeMinedPendingLogs(crit loveblock.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) subscribeLogs(crit loveblock.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) subscribePendingLogs(crit loveblock.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
